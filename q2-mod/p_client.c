...
// 1 - soldier, 2 - pyro, 3 - heavy, 4 - scout, 5 - demoman
void InitClientPersistant (gclient_t *client)
{
	...
	if(client->resp.class == 1){ //are you soldier?
		...
		item = FindItem("Rocket Launcher");	//give rocket launcher
		client->pers.selected_item = ITEM_INDEX(item);
		client->pers.inventory[client->pers.selected_item] = 1;
		client->pers.weapon = item;

		item = FindItem("Rockets"); //give ammo
		...
		item = FindItem("Machinegun");	//give machinegun
		client->pers.selected_item = ITEM_INDEX(item);
		client->pers.inventory[client->pers.selected_item] = 1;
		client->pers.weapon = item;
	...
	else if (client->resp.class == 3) { //are you heavy?
	...
	else if (client->resp.class == 4) { //are you scout?
...
void ClientBeginDeathmatch (edict_t *ent)
{
	...
	gi.bprintf(PRINT_HIGH, "%s entered the game\n", ent->client->pers.netname);
	CheckClass(ent);
	ent->client->perkInitiated = false;
...
void ClientThink (edict_t *ent, usercmd_t *ucmd)
{
	...
	if (ent->client->perkInitiated == true) {

		switch (ent->client->perkType) {
		case 0:
		...
		case 2:
			...
			ent->client->perkHealth = true;
			
			if (ent->client->perkStart == 0) { //if timer is exactly at 15 seconds
				ent->max_health += ent->max_health; //add health boost ONLY ONCE
				ent->health = ent->max_health;
				ent->client->perkStart = 1;
			}

			break;
		case 3:
			gi.centerprintf(ent, "Explosive Bullets!!\n\n%.1f", ent->client->p_timer);
